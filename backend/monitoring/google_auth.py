...
# Defer importing Google client libraries until they're actually needed so
# the Django app can start even when those optional deps are not installed.
...
def get_google_credentials():
    ...
    except Exception as e:  # ImportError or missing package
        raise ImportError(
            "Google auth libraries are not installed. Install with: `pip install google-auth google-auth-oauthlib google-api-python-client`"
        ) from e
...
def get_drive_service():
    ...
    try:
        ...
        raise ImportError(
            "Google API client libraries are not installed. Install with: `pip install google-api-python-client`"
        ) from e
